...
from .trams import readTramNetwork, TramNetwork, json2data
...
def stop_url(stop):
    ...
    f = json2data('..\stop_timetable.json')
    return f[stop]['timetable']
...
def show_shortest(dep, dest):
    ...
    network_geo = readTramNetwork(geo=True)
    # f = json2data(TRAM_FILE)
    # network = TramNetwork(f['lines'], f['stops'], f['times'])
    # spec_network = specialize_stops_to_lines(network)
    # geo_path = specialized_geo_distance(spec_network, dep, dest)
    # time_path = specialized_transition_time(spec_network, dep, dest)

    time_path = dijkstra(network, dep)[dest]
    geo_path = dijkstra(network_geo,dep)[dest]
    ...
    for i in time_path:
        if i == dest:
            ...
            t_path = t_path + i + '-'
    timepath = 'The quickest route from ' + dep + ' to ' + dest + ' ' + t_path
    ...
    for i in geo_path:
        if i == dest:
            ...
            g_path = g_path + i + '-'
    geopath = 'The shortest route from ' + dep + ' to ' + dest + ' ' + g_path

    # TODO: run this with the shortest-path colors to update the svg image
    def stops_colors(stop):
        if stop in t_path and stop in g_path:
            return 'cyan'
        elif stop in t_path:
            return 'orange'
        elif stop in g_path:
    ...
    # , stops_colors
    network_graphviz(network, SHORTEST_PATH_SVG, stops_colors)
